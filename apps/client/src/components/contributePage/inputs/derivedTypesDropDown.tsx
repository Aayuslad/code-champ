...
import { derivedTypes } from "../../../pages/Contribute";

type props = {
    options: typeof derivedTypes;
    selectedOption: (typeof derivedTypes)[number];
    setSelectedOption: (value: (typeof derivedTypes)[number]) => void;
};

function DerivedTypesDropDown({ options, selectedOption, setSelectedOption }: props) {
    const [isOpen, setIsOpen] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);

    const toggleDropdown = () => setIsOpen(!isOpen);

    const handleOptionClick = (option: (typeof derivedTypes)[number]) => {
    ...
    return (
        <div className="relative inline-block w-fit" ref={dropdownRef}>
            <button
                ...
                className="pt-1 pb-1 px-2 border-2 flex justify-between gap-1 border-light300 dark:border-dark300 w-[130px] rounded-lg"
            >
...
export default DerivedTypesDropDown;
